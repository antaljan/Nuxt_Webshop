  ...
  >
    <div class="relative z-10 container mx-auto max-w-4xl px-4">
      ...
      <div v-if="isAdmin && !editTextMode && !editDesignMode" class="mt-6 flex flex-wrap gap-3">
        <v-btn class="admin-primary" @click="openTextEditor">Szöveg szerkesztése</v-btn>
        <v-btn class="admin-secondary" @click="openDesignEditor">Szakasz stílusa</v-btn>
      </div>

      <!-- DESIGN EDITOR -->
      <div v-if="isAdmin && editDesignMode" class="mt-6 p-5 rounded-xl border bg-white shadow-inner space-y-5 text-black">
        ...
        <!-- BACKGROUND COLOR -->
        <div>
          <p class="text-sm font-semibold text-gray-600 mb-2">Szakasz háttérszíne:</p>
        ...
        <!-- WAVES -->
        <div>
          <p class="text-sm font-semibold text-gray-600 mb-2">Szakasz lezáró él (Forma):</p>
          <div class="flex flex-wrap gap-3">
            <button
        ...
        <div v-if="localContent.wave" class="space-y-3 pt-2">
          <p class="text-sm font-semibold text-gray-600">Következő szakasz színe (Hullám kitöltése):</p>
...
/* -------------------------------------------------------------
   SZERKESZTŐ MÓDOK SZÉTVÁLASZTVA
------------------------------------------------------------- */
const editTextMode = ref(false)
const editDesignMode = ref(false)

/* -------------------------------------------------------------
   LOKÁLIS TARTALOM (mindkét editor ezt módosítja)
------------------------------------------------------------- */
...
/* -------------------------------------------------------------
   EREDETI TARTALOM MENTÉSE KÜLÖN-KÜLÖN
------------------------------------------------------------- */
const originalTextContent = ref(null)
const originalDesignContent = ref(null)
...
/* -------------------------------------------------------------
   SZÖVEG EDITOR MEGNYITÁSA / MENTÉSE / MÉGSE
------------------------------------------------------------- */
const openTextEditor = () => {
  originalTextContent.value = JSON.parse(JSON.stringify({
...
const saveText = async () => {
  await updateSection(props.sectionKey, locale.value, {
    ...activeContent.value,
    html: localContent.value.html
  })
...
/* -------------------------------------------------------------
   DESIGN EDITOR MEGNYITÁSA / MENTÉSE / MÉGSE
------------------------------------------------------------- */
const openDesignEditor = () => {
  originalDesignContent.value = JSON.parse(JSON.stringify({
...
const cancelDesign = () => {
  if (originalDesignContent.value) {
    Object.assign(localContent.value, originalDesignContent.value)
  }
  editDesignMode.value = false
}
